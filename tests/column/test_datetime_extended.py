...
def test_dt_is_special_days(datetime_extended_df):
    ...
    assert result.iloc[0]["is_month_start"] == True
    assert result.iloc[0]["is_quarter_start"] == True
    assert result.iloc[0]["is_year_start"] == True
    
    # Verificações de último dia do mês/ano (31/12/2023)
    # Agora sem a parte de tempo (00:00:00), deve funcionar corretamente
    assert result.iloc[1]["is_month_end"] == True
    assert result.iloc[1]["is_year_end"] == True
    
    # Verificações de último dia do mês (31/03/2023)
    assert result.iloc[3]["is_month_end"] == True
